...
# Make a dataframe with participants and their benet
...
# Define priors for non-scaled variables
prior_mod_1 <- c(
...
# Scaled priors
#prior_mod_1 <- c(
#  prior(normal(0, 1),     class = b), #mean and sd of calibration
#  prior(normal(1, .5),    class = sigma) #mean: sd of calibration. sigma: sd of calibration/2
#)
...
# Define priors
prior_mod_2 <- c(
  prior(normal(0, .12),  class = b), #mean and sd of sus
  prior(normal(.12, .06),   class = sigma) #mean: sd of sus. sigma: sd of sus/2
)

# Scaled priors
#prior_mod_2 <- c(
#  prior(normal(0, 1),     class = b), #mean and sd of sus
#  prior(normal(1, .5),   class = sigma) #mean: sd of sus. sigma: sd of sus/2
#)
...
# Define priors for non-scaled varibles
prior_mod_3 <- c(
...
# Priors non-scaled without participant 39
#prior_mod_3 <- c(
...
# Scaled priors
#prior_mod_3 <- c(
#  prior(normal(0, 1),     class = b), #mean and sd of Benefit 
#  prior(normal(1, .5),    class = sigma) #mean: sd of Benefit. sigma: sd of Benefit/2.
#)
...
# Scale susceptibility and calibration
...
# Scaled priors
#prior_mod_4 <- c(
#  prior(normal(0, 1),     class = b), #mean and sd of outcome
#  prior(normal(0, 1),     class = Intercept), #mean and sd of outcome
#  prior(normal(1, .5),    class = sigma) #mean: sd of outcome. sigma: sd of outcome/2.
#)
...
plot(hypothesis(mod_4_1,"Intercept > 0")) # plot model learning of intercept (which shows no prior though...)
...
# Remove participant 39, because it is an outlier
#df_5 <- df_5[-c(9121:9360), ]
...
# Define unscaled priors without participant 39
#prior_mod_5 <- c(
#  prior(normal(6.71, 30.5),     class = b), #mean and sd of benefit
#  prior(normal(6.71, 30.5),     class = Intercept), #mean and sd of benefit
#  prior(normal(30.5, 15.25),    class = sigma) #mean: sd of benefit. sigma: sd of benefit/2
#)
...
# Look at the average benefit of each participant, in z-scores - participant 39 definitely looks like an outlier
df_meta$Ben_z_scores <- scale(df_meta$Benefit, center = TRUE, scale = TRUE)
